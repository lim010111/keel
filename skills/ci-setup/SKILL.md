...
**런타임 핀 파일을 매트릭스와 같이 결정한다** — local과 CI가 같은 series를 쓰지 않으면 "로컬 green = CI green"의 신뢰가 깨진다. 매트릭스의 "개발용 기본 버전"에 맞춰 언어별 핀 파일을 repo에 commit한다:

| 언어 | 핀 파일 | 비고 |
|---|---|---|
| Python | `.python-version` | uv / rye / pyenv 인식. major.minor (`3.13`)면 patch 자동 롤포워드, full semver (`3.13.11`)는 정확 핀. |
| Node | `.nvmrc` | Volta / fnm / nvm. `package.json`의 `engines.node`로도 가능. |
| Rust | `rust-toolchain.toml` | `[toolchain] channel = "1.78"`. |
| Go | (`go.mod`의 `go` 디렉티브로 충분 — 별도 파일 없음) | |
| Ruby | `.ruby-version` | |

기존 핀 파일이 있으면 매트릭스와 일치하는지 확인하고 어긋나면 사용자에게 보고. 핀 파일은 G5의 미리보기에 포함해서 같이 승인받는다.
...
### G5. YAML 미리보기 → 액션 태그 검증 → 쓰기
1. G2~G4 결정에 기반해 `ci-<language>.yml`들 + G3의 런타임 핀 파일들을 메모리상 조립.
2. **모든 워크플로에 무조건 포함**:
   - `concurrency: { group: ${{ github.workflow }}-${{ github.ref }}, cancel-in-progress: true }`
   - `permissions: { contents: read }` (도구가 더 필요로 할 때만 상향)
   - 의존성 캐싱 (`setup-python` / `setup-node` 등의 `cache:` 옵션)
3. **액션 태그 검증** — 조립한 YAML의 모든 `uses: <owner/repo>@<tag>`를 `scripts/verify_action_tag.sh <owner/repo>@<tag>`로 돌린다 (read-only, `git ls-remote` 기반, 인증 불필요). 모든 태그가 OK일 때까지 진행하지 않는다.
   - 미해결 태그가 있으면 스크립트가 stderr로 `FALLBACK <owner/repo>@<tag>`를 알려준다. 사용자에게 미해결 태그 + fallback 후보를 보고하고 YAML을 수정한 뒤 **재검증**한다.
   - **검증되지 않은 태그는 절대 쓰지 않는다.** `vN` rolling major가 끊긴 액션이 늘고 있다 (예: `astral-sh/setup-uv`는 v8.0.0부터 immutable tag 정책 — `@v8`은 없고 `@v8.1.0`만 있음).
4. 기존 `.github/workflows/*.yml`이 있으면 **diff를 보여주고** 덮어쓸지 묻는다. G3의 핀 파일도 같이 미리보기에 포함.
5. 승인 시 실제로 파일 작성. 거절 시 자연어 피드백 → 해당 부분만 재조립 후 다시 미리보기.
...
- Action 버전은 G5 검증을 통과한 가장 짧은 핀을 쓴다 — 메이저 핀이 verify되면 메이저 (`actions/checkout@v4`), 안 되면 해당 메이저 안 최신 semver (`astral-sh/setup-uv@v8.1.0`). SHA 핀은 사용자가 명시 요청 시에만.
...
- G5의 액션 태그 검증을 건너뛰고 YAML 쓰기 — 메이저 핀은 *추정*이지 *검증*이 아니다.
- G3의 런타임 핀 파일 없이 매트릭스만 쓰기 — local ↔ CI drift는 첫 PR에서 안 드러나도 미래에 silent green을 만든다.
