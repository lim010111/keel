...
5. **저장 경로 계산** — JSON의 `logical_date`와 `display_hhmm`를 그대로 사용:
   ```
   /mnt/c/Users/shine/Documents/Obsidian/0. Daily/Dev log/<YYYY>/<YYYY-MM>/<logical_date>/Sessions/<display_hhmm> <제목>.md
   ```
   - `logical_date`: 세션이 *문맥상* 속한 날 (YYYY-MM-DD). 00:00~04:59 사이에 시작한 새벽 세션은 전날 폴더로 귀속된다.
   - `display_hhmm`: 파일명용 시각. 새벽 세션은 24+ 시간으로 표기되어 같은 폴더의 저녁 세션 뒤로 정렬된다 (예: 01:30 시작 → `25-30`).
...
- [ ] 헤더 시각이 KST인가
- [ ] 파일이 `<YYYY>/<YYYY-MM>/<logical_date>/Sessions/<display_hhmm> <제목>.md` 경로에 저장되었나 (새벽 세션은 전날 폴더 + 24+ 시간 prefix)
